<script>
	...
	let notificationSettings = $state(
		settingsStore.data?.notifications || {
			tasks: {
				enabled: false,
				before: 30, // minutes
				sound: true
		...
		<div class="rounded-lg bg-blue-50 p-4">
			<div class="flex">
				...
				<div class="ml-3">
					...
					<div class="mt-4">
						<button type="button" class="btn-primary" onclick={requestPermission}>
		...
		<div class="space-y-6">
			...
			<div>
				<div class="flex items-center justify-between">
					...
					<button
						type="button"
						class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 {notificationSettings
							.tasks.enabled
							? 'bg-primary-600'
							: 'bg-gray-200'}"
						role="switch"
			...
			<div class="border-t pt-6">
				<div class="flex items-center justify-between">
					...
					<button
						...
						onclick={() => {
							notificationSettings.studyReminders.enabled =
								!notificationSettings.studyReminders.enabled;
							updateSettings();
						}}
						disabled={permissionStatus !== 'granted'}
